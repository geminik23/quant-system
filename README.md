...
- `qs-strategy` provides a reusable synchronous configured strategy core with typed parameter templates, factory-declared material schemas, branch pruning, bounded logical bar sources, derived source-specific input requirements, composable indicator primitives, typed bounded expressions, deterministic material and finite-state evaluation, total vacant/pending/open trade-slot facts including open-position time, excursion, and initial risk, calendar materials, generic decisions and notes, and validated command-correlated strict `RawSignal` values, where an Entry may carry an optional entry class for adapter-owned profile routing. It remains library-only and owns no historical feeds, services, live runtime, persistence, or management-profile composition.
- `qs-backtest` provides validated historical strategy contracts and a configured-strategy adapter over the existing FutureQuote scheduler. The adapter performs complete logical-source binding from ticks or from stored bars of the declared timeframe, exact tick-count volume projection, named-input projection, total trade-slot projection, ordered command provenance and committed feedback, final feedback processing, decision and note mapping, and management-profile selection shared with raw-signal replay: an entry routes by its entry class or falls back to the run default profile, and an unrouted class or a profile that would manage a stop the strategy also moves is rejected before feed consumption.

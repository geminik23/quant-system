...
- `qs-backtest` provides validated historical strategy contracts, bounded causal fixed-duration closed-bar series, complete-boundary causal analysis, and stateful callback contracts with read-only series, observations, engine state, and borrowed execution facts. Dynamic strategy execution through FutureQuote is not implemented yet.

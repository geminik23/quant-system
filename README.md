...
- `qs-backtest` provides validated historical strategy contracts, bounded causal fixed-duration closed-bar series, and a complete-boundary analysis layer with typed observations, causal annotations, bounded read-only history, and confirmed pivots. Stateful historical strategy callbacks and dynamic FutureQuote strategy integration are not implemented yet.

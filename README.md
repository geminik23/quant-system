...
- `qs-backtest-client` provides provider-neutral submit-once retained execution, same-job status-first reconnect, bounded inline/artifact verification, no-clobber Result Documents, offline reopen and optional typed local analysis. The default graph remains free of xrpc and the backtest engine; shipped provider bindings use the typed xrpc facade without exposing method names or provider errors.

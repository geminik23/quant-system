...
`qs-backtest-client` owns provider-neutral desktop endpoint validation, managed catalog and retained-run connectors, bounded strict signal inspection, typed request preflight, submit-once retained execution, accepted-job status-first reconnect, cancel/detach/shutdown, verified inline and artifact delivery, no-clobber versioned Result Documents, offline reopen, typed execution projections and optional background local analysis. Its default normal dependency graph remains free of xrpc and the backtest engine; the optional xrpc connector and analysis engine dependency are isolated by feature.

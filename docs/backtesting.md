...
This capability is library-only. Applications own configuration file loading, input adaptation, command execution, and persistence. `qs-backtest` does not yet provide the historical configured adapter that would bind logical sources to historical series and retained history, project completed bars, selected observations, readiness, immutable position facts, and committed FutureQuote effects and dispositions into the reusable input, and preserve configured command IDs through scheduling. Configured historical execution through the existing journal, accounting, MTM, and report path is therefore unavailable, as are server or RPC execution, live runtime orchestration, persisted configured state, and configured-strategy composition with `ManagementProfile`.

//! A minimal single-line text input that supports IME composition.
//!
//! Adapted from the Zed GPUI `input` example. It exists so the Windows
//! foundation gate can verify Korean IME input in the static shell. The
//! input owns no business state; the preview shell only displays it.
...
actions!(
    preview_text_input,
    [Backspace, Delete, Left, Right, SelectAll, Home, End, Paste]
);

pub struct PreviewTextInput {
...
impl PreviewTextInput {
    pub fn new(cx: &mut Context<Self>, placeholder: &str) -> Self {
        Self::new_with_content(cx, placeholder, "")
    }
    ...
    fn delete(&mut self, _: &Delete, window: &mut Window, cx: &mut Context<Self>) {
        if self.selected_range.is_empty() {
            ...
            if self.selected_range.end == next {
                return;
            }
...
impl EntityInputHandler for PreviewTextInput {
    fn text_for_range(
        &mut self,
...
impl Render for PreviewTextInput {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        div()
            ...
            .bg(white())
            .border_1()
            .border_color(crate::view::theme::border())
            .rounded_md()
            .px_2()
            .text_size(px(crate::view::theme::TEXT_SIZE))
            .line_height(px(22.))
            .w_full()
...
impl Focusable for PreviewTextInput {
...
/// Bind the editing keys used by the preview input. Registered once at app
/// startup next to the shell bindings.

...
enum LocalCommand {
    // Buy { symbol: String, quantity: u32 },
    // Sell { symbol: String, quantity: u32 },
    Ping(String),
}
...
impl quant::core::CommandHandler<LocalCommand> for Handler {
    async fn handle(&mut self, cmd: LocalCommand) {
        match cmd {
            // LocalCommand::Buy { symbol, quantity } => {
            //     log::info!("Handle Buy {quantity} x {symbol}");
            // }
            // LocalCommand::Sell { symbol, quantity } => {
            //     log::info!("Handle Sell {quantity} x {symbol}");
            // }
...
async fn main() -> Result<()> {
    ...
    let _config = load_config::<Config>(&cli.config)?;
